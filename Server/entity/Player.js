...
const { Utils, managerToFetchingStrategyOptions } = require("discord.js");
...
const { Biomes } = require("../enums/Biomes");
const { MountType } = require("../enums/MountType");
...
class Player extends Entity_1.Entity {
    ...
    wand1 = 0;
    wand2 = 0;
    constructor(controller, id, gameServer, tokenScore, token, token_id, name = 'Logixx', skin = 0, accessory = 0, book = 0, box = 0, baglook = 0, deadBox = 0) {
        ...
        this.packetInterval = setInterval(() => {
            if (this.direction != MovementDirection_1.MovementDirection.STATIC) {

                this.checkPackets(this.movementPacket, 55, PacketType_1.ServerPacketTypeJson.Kick, '[ZMA]: Please Turn Off Script!', this.packetInterval);

                this.movementPacket = 0;
    ...
    survivalUpdate() {
        ...
        if (now - this.lastSurviveUpdate > serverconfig_json_1.default.other.dayInMilliseconds) {
            ...
            writer.writeUInt8(PacketType_1.ServerPacketTypeBinary.Survive, this.ownerClass.id);
                ...
                case itemsmanager_1.ItemMetaType.EMERALD:
                    if (!this.Emerald_Spawn) {
                        if (Math.floor(this.x / 100) == trade.position[0] && Math.floor(this.y / 100) == trade.position[1] && !this.isFly && !this.ghost) {
                            ...
                            this.Emerald_Spawn = true;
                            this.gameServer.WorldGenerator.ReplaceEmerald(true, this.gameServer)
                        };
                    };
                    break;

                case itemsmanager_1.ItemMetaType.SPAWN: {
                    if (Math.abs((this.x / 100) - trade.position[0]) < 1 && Math.abs((this.y / 100) - trade.position[1]) < 1 && !this.ghost && !this.isFly) {

                        let _required = true;

                        for (let i = 0; i < trade.authenticate.length; i++) {
                            let theItem = trade.authenticate[i]
                            if (theItem.length < 1) {
                                continue;
                            };

                            let name = ItemIds_1.ItemIds[trade.authenticate[i][0]]
                            const amount = this.inventory.countItem(name) >= trade.authenticate[i][1];

                            if (!amount) _required = false;
                        ...
                        for (let i = 0; i < trade.kit.length; i++) {
                            let give = trade.kit[i]
                            if (give.length < 1) {
                                continue;
                            };

                            let name = ItemIds_1.ItemIds[trade.kit[i][0]];
                            const amount = trade.kit[i][1]
                    ...
                    if (Math.floor(this.x / 100) == trade.position[0] && Math.floor(this.y / 100) == trade.position[1] && !this.ghost) { // !this.isFly &&
                        ...
                        if (trade.give) {
                            for (let i = 0; i < trade.give.length; i++) {
                                let give = trade.give[i]
                                if (give.length < 1) {
                                    continue;
                                };
                                let name = ItemIds_1.ItemIds[give[0]];
                                const item = this.inventory.containsItem(name);
                                const amount = this.inventory.countItem(name) >= give[1];
                                if (!item || !amount) {
                                    _required = false;
                                    continue;
                                }
                                this.inventory.addItem(name, give[1]);
                            }
                        }
    ...
    callEntityUpdate(isHard) {
        ...
        writer.writeUInt16(PacketType_1.ServerPacketTypeBinary.EntityUpdate, this.ownerClass.id);
        for (let i = 0; i < entities.length; i++) {
            const entity = entities[i];
            if (entity.Invisible && entity.id !== this.ownerClass.id) {
                continue;
            };

            if (this.stealToken && this.stealToken.on) {
                writer.sendSteal(this.stealToken.target, this.stealToken.ownerclass.ownerClass);
            };

            writer.writeUInt8(entity.playerId % 255, this.ownerClass.id);
            writer.writeUInt8(entity.angle % 255, this.ownerClass.id);
            writer.writeUInt16(entity.action, this.ownerClass.id);
            writer.writeUInt16(entity.type, this.ownerClass.id);
            writer.writeUInt16(entity.x, this.ownerClass.id);
            writer.writeUInt16(entity.y, this.ownerClass.id);
            writer.writeUInt16(entity.id, this.ownerClass.id);
            writer.writeUInt16(entity.info, this.ownerClass.id);
            writer.writeUInt16(entity.speed < 2 ? 0 : entity.speed * 10, this.ownerClass.id);
            writer.writeUInt16(entity.extra, this.ownerClass.id);
        };
    ...
    getVechile(vech) {
        var _ = false;
        _ = vech === 0 ? false : null;
        _ = (vech === 221 || vech === 222 || vech === 223 || vech === 224 || vech === 91) ? true : _;
        return _;
    }
    updateMovement(direction) {
        ...
        if (this.ownerClass.ISF && this.isFly && !this.gliding) {
            this.onMap = true;
        } else {
            this.onMap = false;
        };

        if (this.x / 100 >= 61 && this.x / 100 <= 67 && this.y / 100 >= 12 && this.y / 100 <= 18) { // Spawn area
            this.onMap = true;
        }

        if (this.x / 100 >= 90 && this.x / 100 <= 97 && this.y / 100 >= 30 && this.y / 100 <= 34) { // Flower AFK area
            this.onMap = true;
        }


        const HandItem = itemsmanager_1.ItemUtils.getItemById(this.right);
        const isWeapon = (HandItem.meta_type == itemsmanager_1.ItemMetaType.SWORD || HandItem.meta_type == itemsmanager_1.ItemMetaType.BOW);

        if (this.onMap && this.speed < 20 && this.getVechile(this.extra) === true && direction === MovementDirection_1.MovementDirection.STATIC) { // Stuck in any object
            this.pushPrev = !this.pushPrev ? this.realPrev : this.realPrev = this.pushPrev ? this.realPrev : this.pushPrev;
            this.new_speed = 20;
            speed = this.speed;
        } else
            if (this.onMap && this.speed < 21 && this.getVechile(this.extra) === true && direction !== MovementDirection_1.MovementDirection.STATIC) {
                this.new_speed = 21;
                speed = this.speed;
            }
        ...
        function pushOutOfObj(direction) {
            switch (direction) {
        ...
        switch (direction) {
            ...
            case MovementDirection_1.MovementDirection.LEFT_TOP:
                ...
                vectorY -= speed * deplifier;
                break;
            case MovementDirection_1.MovementDirection.STATIC:
                this.realPrev = this.prevDirection !== 0 ? this.prevDirection : this.realPrev;

                const mount = itemsmanager_1.ItemUtils.getItemById(this.extra)
                const decrease_mount_speed = mount.data.slowSpeed;

                if (this.stateManager.isInRoof == true && this.isFly && !this.gliding && this.getVechile(this.extra) === true || this.onMap === true && this.getVechile(this.extra) === true) { // over rocks or objects
                    this.speed < 20 ? (this.speed = 20, this.gliding = true) : null;
                    switch (this.realPrev) {
                        case MovementDirection_1.MovementDirection.LEFT:
                            vectorX -= this.speed;
                            break;
                        case MovementDirection_1.MovementDirection.RIGHT:
                        ...
                        case MovementDirection_1.MovementDirection.BOTTOM:
                            ...
                            break;
                        case MovementDirection_1.MovementDirection.LEFT_BOTTOM:
                            vectorX -= this.speed * deplifier;
                            vectorY += this.speed * deplifier;
                            break;
                        case MovementDirection_1.MovementDirection.RIGHT_BOTTOM:
                            vectorX += this.speed * deplifier;
                            vectorY += this.speed * deplifier;
                    ...
                    if (this.getVechile(this.extra) === true && this.speed > 1 && !this.gliding) {
                        this.speed = this.speed - decrease_mount_speed;
                ...
                break;
            default:
                this.direction = this.invalidPacket(direction, 'movement');
        ...
        if (pos1.x === 0 && pos2.x === 0 && pos1.y === 0 && pos2.y === 0 && this.isFly) {
            this.direction = this.realPrev;
        }
        ...
        if (this.speed > 17 && this.getVechile(this.extra) === true) {
            this.isFly = true;
        } else {
    ...
    invalidPacket(pack, type) {
        switch (type) {
            case 'movement':
                return this.realPrev;
                break;

            default:
                console.log('Unkown Type', type, pack)
                break;
        }
    }

    checkPackets(a, b, c, d, e) {
        if (a > b) {
    ...
    updateDirection(direction) {
        ...
        if (direction == 0) {
            this.vector.y = 0; this.vector.x = 0;
    ...
    tickUpdate() {
        let old_speed = this.old_speed;
        let max_speed = this.max_speed;
        this.new_speed = max_speed;
        let Increase_water = 0;

        const HandItem = itemsmanager_1.ItemUtils.getItemById(this.right);
        const isWeapon = (HandItem.meta_type == itemsmanager_1.ItemMetaType.SWORD || HandItem.meta_type == itemsmanager_1.ItemMetaType.BOW)

        if (HandItem != null && isWeapon) {
            this.new_speed = serverconfig_json_1.default.entities.player.speed_weapon;
        }
        if (this.biomeIn == Biomes.SEA && !this.stateManager.isInBridge && !this.isFly) {
            this.new_speed = serverconfig_json_1.default.entities.player.speed_water;
            switch (this.hat) {
                case ItemIds_1.ItemIds.DIVING_MASK:
                    this.new_speed = serverconfig_json_1.default.entities.player.speed_water_diving;
                    Increase_water = 2;
                    break;
                case ItemIds_1.ItemIds.SUPER_DIVING_SUIT:
                    this.new_speed = serverconfig_json_1.default.entities.player.speed_water_suit;
                    Increase_water = 4;
                    break;
            }
            if (isWeapon) {
                this.new_speed = serverconfig_json_1.default.entities.player.speed_water_weapon + Increase_water;
            }
        }
        if (this.biomeIn == Biomes.WINTER && !this.isFly) {
            this.new_speed = serverconfig_json_1.default.entities.player.speed_winter;
            if (isWeapon)
                this.new_speed = serverconfig_json_1.default.entities.player.speed_winter_weapon;
        }
        if (this.ownerClass.ISF && !this.isFly) {
            this.new_speed = serverconfig_json_1.default.entities.player.collide_speed;
        }
        if (this.direction == MovementDirection_1.MovementDirection.STATIC && this.extra == MountType.NONE) {
            this.new_speed = serverconfig_json_1.default.entities.player.mount_equip_reset;
        }

        if (this.extra === MountType.NONE) {
        ...
        if (this.extra != MountType.NONE) {
            ...
            const mount = itemsmanager_1.ItemUtils.getItemById(this.extra);
            const max_mount_speed = Number(mount.data.maxSpeed - (isWeapon ? 6 : 0));
            const increase_mount_speed = mount.data.raiseSpeed;
            const decrease_mount_speed = mount.data.slowSpeed;

            const isPilot = this.hat == ItemIds_1.ItemIds.PILOT_HELMET ? (2.25) : (1);

            if (this.direction == MovementDirection_1.MovementDirection.STATIC) {
                this.new_speed = Math.max((old_speed - decrease_mount_speed), 1);
            }
            ...
            if (this.direction != MovementDirection_1.MovementDirection.STATIC) {
                ...
                if (!this.startFlying) {
                    ...
                    if (!this.isAdmin && !this.isMod && !this.staffAllowed) {
                        old_speed = 1;
                    } else
                        if (this.isOwner) {
                            old_speed = max_mount_speed;
                        };
                };

                this.new_speed = Math.min((old_speed + increase_mount_speed * isPilot), this.toggledSpeed ? this.max_speed : max_mount_speed);
            }
        }
        if (this.direction != MovementDirection_1.MovementDirection.STATIC) { this.movementPacket++ };
        this.speed = this.checkStun(this.new_speed);
        this.old_speed = this.speed
        this.updateMovement(this.direction);
