...
async function run() {
    ...
    if (cmd === "analyze-dts") {
        ...
        const source = ts.createSourceFile("bundle.d.ts", dtsCode, ts.ScriptTarget.Latest, true);
        ...
        if (args.values.builtins) {
            ...
            for (const builtinName of args.values.builtins) {
                if (builtinName in builtins) {
                    console.log("[optools:include-builtins]", builtinName, builtins[builtinName].length);
