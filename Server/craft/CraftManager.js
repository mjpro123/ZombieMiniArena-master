...
class CraftManager {
    ...
    handleRecycle(itemId) {
        ...
        if (craftBound) {
            ...
            writer.writeUInt8(PacketType_1.ServerPacketTypeBinary.RecycleOk, this.player.id);
            writer.writeUInt8(craftId, this.player.id);
    ...
    cancelCraft() {
        if (this.isCrafting() || this.isRecycling()) {
            ...
            writer.writeUInt8(PacketType_1.ServerPacketTypeBinary.CancelCraft, this.player.id);
            this.player.controller.sendBinary(writer.toBuffer());
            this.toCraft = null;
            this.toRecycle = null;
            this.elapsed = -1;
    ...
    handleCraft(craftId) {
        ...
        if (craftBound) {
            ...
            craftResponse.writeUInt8(PacketType_1.ServerPacketTypeBinary.CraftOk, this.player.id);
            craftResponse.writeUInt8(craftId, this.player.id);
