...
class StorageEvents {
    static takeItemFromChest(packetData_, cn) {
        if (!packetData_ || !cn) return;
        if (packetData_.length < 2) return;
        const chestPlayerId = packetData_[0], chestId = packetData_[1];
        const chest = cn.gameServer.getEntity(chestId);
        if (!chest || cn.sourcePlayer.isFly || !chestPlayerId || !chestId)
            return;
        if (!chest.data || chest.playerId == undefined || chest.is_locked == undefined)
            return;
        ...
        if (chest.Monitor) {
            DiscordBot.Logs(chest.owner, null, cn.sourcePlayer, 'Chest', null, null, currentContent)
            chest.Monitor = false;
        }
        if (cn.sourcePlayer.inventory.countItem(currentContent[0]) + currentContent[1] > 10) {
            const item = itemsmanager_1.ItemUtils.getItemById(currentContent[0])
            if (item && item.meta_type == itemsmanager_1.ItemMetaType.ARROW)
                return;
        }
    ...
    static addItemChest(packetData_, cn) {
        ...
        if (Utils_1.Utils.isAdmin(cn.sourcePlayer)) {
            chest.Monitor = true;
            chest.owner = cn.sourcePlayer
        }
    }
